    ...
    def portal_paymenthistory(self, search='', search_in='all', **kw):
        ...
        for search_paymenthistory in search_paymenthistories:
            ...
            if search_paymenthistory.code:
                paymenthistory['amount'] = search_paymenthistory.line_ids.filtered(lambda line: line.salary_rule_id.code == search_paymenthistory.code).mapped('amount')[0] if (len(search_paymenthistory.line_ids.ids) > 0 and len(search_paymenthistory.line_ids.filtered(lambda line: line.salary_rule_id.code == search_paymenthistory.code)) > 0) else 0.0
            else:
                paymenthistory['amount'] = search_paymenthistory.line_ids.mapped('amount')[0] if len(search_paymenthistory.line_ids.ids) > 0 else 0.0
            paymenthistory['number_of_hours'] = sum(search_paymenthistory.worked_days_line_ids.mapped('number_of_hours')) if len(search_paymenthistory.worked_days_line_ids.ids) > 0 else 0.0
