...
class PortalAccount(portal.CustomerPortal):
    ...
    def portal_accountbalance(self, search='', search_in='all', selected_month='0', **kw):
        if selected_month not in ['0', '1', '2', '3', '4']:
            selected_month = '0'
        # Utilisation de la fonction du helper
        search_accountbalances, searchbar_inputs = Helpers.accountbalance(search, search_in, selected_month)
        total_rate = 0.0
        total_number_of_hours = 0.0
        accountbalances = []
        shared_subjects = {}
        for search_accountbalance in search_accountbalances:
            if not search_accountbalance.date or not search_accountbalance.day_of_week:
                continue
    ...
    def portal_subjectsession_list(self, classe=None, subject=None, search='', search_in='all', **kw):
        ...
        for search_subjectsession in search_subjectsessions:
            if not search_subjectsession.date or not search_subjectsession.day_of_week:
                continue
            subjectsession = {}
