...
class UserAudit(models.Model):
    ...
    def create_audit_log_for_create(self, res_model):
        ...
        audit = self.sudo().search([('model_ids', '=', model.id)], limit=1)
        if audit and audit.is_create:
            self.env['user.audit.log'].sudo().create({
    ...
    def create_audit_log_for_delete(self, res_model, record_id):
        ...
        model = self.env['ir.model'].sudo().search([('model', '=', res_model)], limit=1)
        record_id = self.env[res_model].sudo().browse(record_id)
        audit = self.sudo().search([('model_ids', '=', model.id)], limit=1)
        if audit and audit.is_delete and record_id:
            self.env['user.audit.log'].sudo().create({
                'user_id': self.env.user.id,
                'model_id': model.id,
                'record': record_id,
