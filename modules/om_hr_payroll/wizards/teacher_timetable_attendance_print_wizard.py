...
class TeacherTimetableAttendancePrintWizard(models.TransientModel):
    ...
    def print_teacher_timetable_attendance_report_data(self, resume=False, domains=None):
        ...
        for search_teacher_timetable_attendance in search_teacher_timetable_attendances:
            ...
            teacher_timetable_attendance['class_group_id'] = search_teacher_timetable_attendance.class_group_id.id if search_teacher_timetable_attendance.class_group_id.id else None
            teacher_timetable_attendance['class_group_name'] = search_teacher_timetable_attendance.class_group_id.name if search_teacher_timetable_attendance.class_group_id.id else ''
            ...
            teacher_timetable_attendance['done'] = search_teacher_timetable_attendance.done
            teacher_timetable_attendance['awaiting'] = search_teacher_timetable_attendance.awaiting
            teacher_timetable_attendance['worked_done'] = search_teacher_timetable_attendance.worked_done
            teacher_timetable_attendance['worked_awaiting'] = search_teacher_timetable_attendance.worked_awaiting
            if teacher_timetable_attendance['worked_done'] > teacher_timetable_attendance['hours_credit']:
        ...
        for key in key_teacher_timetable_attendances.keys():
            if key_teacher_timetable_attendances[key]['has_allowance_cd']:
                ...
                if employee_salary_allowance:
                    ...
                    teacher_timetable_attendance['class_group_name'] = ''
                    teacher_timetable_attendance['subject_name'] = ''
                    teacher_timetable_attendance['identifier'] = ''
                    ...
                    teacher_timetable_attendance['done'] = ''
                    teacher_timetable_attendance['awaiting'] = ''
                    teacher_timetable_attendance['worked_done'] = ''
                    teacher_timetable_attendance['worked_awaiting'] = ''
                    teacher_timetable_attendance['class'] = ''
                    key_teacher_timetable_attendances[key]['amount'] += teacher_timetable_attendance['amount']
                    key_teacher_timetable_attendances[key]['total_amount'] = key_teacher_timetable_attendances[key]['amount']
                    key_teacher_timetable_attendances[key]['data'].append(teacher_timetable_attendance)
    ...
    def format_timetable(data, hours=[]):
        ...
        for d in sorted_data:
            ...
            while TeacherTimetableAttendancePrintWizard.increment_float_time(d['start_time']) < TeacherTimetableAttendancePrintWizard.increment_float_time(d['end_time']):
                if TeacherTimetableAttendancePrintWizard.increment_float_time(d['start_time'], n) == 0.0:
                            ...
                            for j, column in enumerate(df[monday].columns):
                                for k, key in enumerate(timetables[monday].keys()):
                                    if k == d['date'].weekday() + 1:
                                        ...
                                        break
                    d['start_time'] = TeacherTimetableAttendancePrintWizard.increment_float_time(d['end_time'])
                else:
