    ...
    def print_timetable_report_data(self, domains=None):
        ...
        for search_timetable in search_timetables:
            ...
            if not key in timetables:
                ...
                info_timetables[key]['filter'] = self.env['ir.config_parameter'].get_param(f'siantou.ems.timetable.timetable.filter.{self.env.user.id}', '')
            timetable = {}
            ...
            timetable['specialty_name'] = search_timetable.specialty_id.name if search_timetable.specialty_id.id else ''
        ...
        self.env['ir.config_parameter'].set_param(f'siantou.ems.timetable.timetable.filter.{self.env.user.id}', '')
