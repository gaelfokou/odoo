...
class TimetableWizard(models.TransientModel):
    ...
    def generate_timetable(self):
        ...
        for record in self:
            ...
            # Récupérer la liste des departments et les traiter l'une après l'autre
            departments = self.env['hr.department'].search([])
            departments = list(departments)
            for department in departments:
                # Récupérer la liste des filières et les traiter l'une après l'autre
                field_of_studies = self.env['siantou.ems.core.field_of_study'].search([('department_id', '=', department.id)])
