...
class SchoolCourseBatch(models.Model):
    ...
    year_id = fields.Many2one('oe.school.year', string='Année', required=True,
                           default=lambda self: self._default_year_id(),
                           domain="[('active','=',True)]")
    date_start = fields.Date(string='Date de début', required=True, readonly=False,
                             store=True, compute='_compute_date_from_school_year')
    date_end = fields.Date(string='Date de fin', required=True, readonly=False,
                           store=True, compute='_compute_date_from_school_year')
    count_subjects = fields.Integer(string='Nombre de cours', compute='_compute_course_subjects')
