...
class OeSchoolCourse(models.Model):
    ...
    name = fields.Char(string='Cursus', required=True, index=True, translate=True)
...
class SchoolCourseSubjectLine(models.Model):
    ...
    course_id = fields.Many2one('oe.school.course', string='Cursus', required=True, ondelete='cascade', index=True)
    company_id = fields.Many2one('res.company',related='course_id.company_id')
    subject_id = fields.Many2one('oe.school.subject', string='Cours', required=True)
