...
class SchoolTimetable(models.Model):
    ...
    teacher_id = fields.Many2one('hr.employee', 'Enseignant',
                                 store=True,
    ...
    allday = fields.Boolean('Tous les jours', default=False)
    ...
    def _check_duplicate_timetable(self):
        for record in self:
            ...
            if len(timetable_ids) > 0:
                raise UserError("Feuille de temps déjà programmée pour la date et l'heure spécifiées.")
    ...
    def action_timetable_planning(self):
        ...
        action.update({
            'name': 'Planification de l\'emploi du temps',
    ...
    def action_timetable_automatic(self):
        action = self.env.ref('de_school_timetable.action_generatetimetable_wizard').read()[0]
        action.update({
            'name': 'Planification automatique',
            'res_model': 'oe.school.generatetimetable.wizard',
            'type': 'ir.actions.act_window',
        })
        return action
    ...
    def action_classroom_allocation(self):
        ...
        if any(tt.date < fields.Date.today() for tt in timetable_ids):
            raise UserError(_("Certains des horaires sélectionnés ont déjà expiré. Veuillez choisir des horaires actifs."))
        return {
            'name': 'Attribution des salles',
            'view_mode': 'form',
